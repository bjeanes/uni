...
(define [string->tokens message]
  (define [s->t-helper char-list final-list prev-char cur-token]
    (if (empty? char-list) ;; if char-list is empty, then we have converted all of the message to tokens,
    			   ;;  so return the final list of tokens:
    	final-list ;; final-list being returned (CHECK LATER THAT LAST TOKEN IS PASSED WITH THIS LIST)
        (cond 	
        	[(and ;;check that both the previous character and the first character of the remaining message are either a character or a number:
        		...
        		(or 
        		  ...
        		  ;; if tests passed:
        		(s->t-helper (rest char-list) final-list (first char-list) (string-append cur-token (first char-list)))
        	   ]
         	[(and
         	...
         	[else (s->t-helper (rest char-list) (append final-list (list cur-token)) (first char-list) (first char-list))]
           )
    )
   )
