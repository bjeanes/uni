...
(define [encipher message encryption-key]
  ...
  (define [do-encipher msg key encr-msg pos]
        ...
        (do-encipher
         ...
         (append encr-msg
                 (list (integer->printchar 
                       ...
                       ) ;; SWITCH REMAINDER IF IT DOESN'T WORK?
                 ;;;;;;;;;;;; ^^^ APPEND TO THE LIST ^^^ ;;;;;;;;;;;;;;;;
                 (+ pos 1)
                 )
         )
    ...
    (do-encipher 
     (string->list message) (string->list encryption-key) (list) 0))) ;; turn encryption-key and message to lists so that we can deal with one character at a time. pass an empty list as the encrypted message (because none is encrypted yet), and pass 0 as the initialised position.
