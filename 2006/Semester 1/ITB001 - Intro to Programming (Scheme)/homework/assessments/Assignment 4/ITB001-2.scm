...
(define [encipher message encryption-key]
  ...
  (define [do-encipher msg key encr-msg pos]
    ...
    (if (or (equal? (length key) 0) (equal? null key))
        ;; if it is, return unaltered message:
        (list->string msg)
        ;; else, continue with encryption:
    (if (equal? (length msg) (length encr-msg)) ;; once original msg and encr-msg are the same length:
        ;; we know that we have encrypted the entire message and can 
        ;; RETURN it.
        (list->string encr-msg)
                        ...
                        (remainder 
                         (+    ;; add integers together (do the actual encryption)
                          ...
                          (printchar->integer ;; convert key character to integer to encrypt message
                           (list-ref key (remainder pos (length key)))))
                         95))))
         (+ pos 1)))));; increase position marker
  
  (if (or (not (string? message)) (not (string? encryption-key))) 
      (error "Please make sure both the message and encryption-key are strings!")
      (do-encipher 
       (string->list message) 
       (string->list encryption-key) 
       empty 
       0
       )
      )
  ) ;; turn encryption-key and message to lists so that we can deal with one character at a time. pass an empty list as the encrypted message (because none is encrypted yet), and pass 0 as the initialised position.
...
(define [decryption-key encryption-key]
  (define [find-decr-key encr-key decr-key encr-key-length]
    (if (equal? encr-key-length (length decr-key)) (list->string decr-key)
        (find-decr-key
         (rest encr-key)
         (append decr-key (list (integer->printchar (- 95 (printchar->integer (first encr-key))))))
         encr-key-length
         )
        )
    )
  ...
  (find-decr-key (string->list encryption-key) empty (string-length encryption-key))
  ) 
